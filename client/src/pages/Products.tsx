...
const products = [
  {
    ...
    category: 'AI 公司操作系统',
    desc: '用于开户注册、创建客户工作区、安装公司包、配置数字员工、提交项目目标、跟踪 Run 和审核 WorkProduct。',
    details: ['opc.reeftotem.ai', 'AI 公司和公司包', '员工组织和工作间', '交付审核与审计'],
  ...
  },
  {
    ...
    name: 'ReefTotem 小助手',
    category: '个人与企业助手',
    desc: '面向办公辅助、知识问答、任务提醒、资料整理和轻量流程协作。它可以独立成为产品，也可以作为 Hermes Company OS 的入口能力。',
  ...
  },
  {
    ...
    name: '量化交易软件',
    category: '量化研究与交易辅助',
    desc: '面向行情分析、策略研究、回测记录、风险监控和交易流程辅助。该方向必须以工具和风控为边界，不做收益承诺。',
  ...
  },
  {
    ...
    name: '音视频与内容安全',
    category: '企业安全与风控',
    desc: '围绕音视频识别、内容安全、风险事件追踪和企业流程接入形成行业解决方案。',
  ...
  },
  {
    ...
    name: '数字员工与公司能力包',
    category: 'AI 组织能力',
    desc: '把岗位、制度、SOP、工具、验收标准和员工模板沉淀为可安装能力，但真实公司记忆和客户资料必须留在公司实例内。',
        ...
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl">
            ...
            <p className="mt-6 text-lg leading-8 text-muted-foreground">
              ReefTotem 不是只有 Hermes Company OS。官网需要把公司产品组合讲清楚：AI 公司操作系统、小助手、量化交易软件、内容安全能力，以及支撑这些产品的数字员工和公司能力包。
            </p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          <div className="grid gap-6">
            {products.map((product) => {
              ...
              return (
                <article key={product.name} className="rounded-lg border border-border bg-card p-6 shadow-sm">
                      ...
                      <div className="rounded-lg border border-border bg-background p-6">
                        <div className="grid gap-3">
                          {product.details.map((detail, index) => (
                            <div key={detail} className="flex items-center justify-between rounded-md border border-border bg-white px-4 py-3">
        ...
        <div className="container mx-auto grid gap-8 px-4 lg:grid-cols-[0.85fr_1.15fr]">
          <div>
            ...
            <p className="mt-4 text-muted-foreground leading-7">
              官网可以展示完整产品矩阵，但不能把规划中的产品包装成已经完整交付。每个产品都要标注成熟度、适用范围和风险边界。
            </p>
          </div>
          <div className="grid gap-4 md:grid-cols-3">
            {[
              ['已部署控制台', '已有线上入口，可进行真实功能验证。'],
              ['产品线规划', '已有方向和边界，需要继续完成产品定义、页面和交付链路。'],
          ...
          <div className="grid gap-6 rounded-lg border border-border bg-foreground p-8 text-background lg:grid-cols-[1fr_auto] lg:items-center">
            <div>
              ...
              <p className="mt-4 max-w-3xl text-sm leading-6 text-background/75">
                小助手、量化交易软件和内容安全能力后续需要补独立产品页；Hermes Company OS 的操作文档继续放在 OPC 和文档入口中。
              </p>
            </div>
            <Link
              to="/contact"
